...
function EditRecurringDialog({
  ...
  onSubmitEditRecurring, // <-- Debe aceptar SOLO (payload)
  onCancelEdit,
}) {
  ...
  useEffect(() => {
    if (!open) return;
    const onKey = (e) => e.key === "Escape" && onCancelEdit();
  ...
  const updateEditing = (field, value) => {
    ...
    onEditingRecurringChange({ ...editingRecurring, [field]: value });
  };
          ...
          {editingRecurring.category && (
            <div>
              ...
              >
                ...
                {getCategorySubcategories(categories[editingRecurring.category])?.map((sub) => (
...
const RecurringExpensesModal = ({
  ...
  onAddRecurring, // sigue recibiendo el evento del form "Añadir"
  onUpdateRecurring,
  onRequestDelete,
  onStartEdit,
  editingRecurring,
  onEditingRecurringChange,
  onSubmitEditRecurring, // debe aceptar (payload)
  onCancelEdit,
  onClose,
}) => {
  const { t } = useTranslation();
  ...
  const updateNew = (field, value) => {
    onNewRecurringChange({ ...newRecurring, [field]: value });
  };
  ...
  const getFrequencyDisplayText = (recurring) => {
    const frequency = recurring.frequency || "monthly";
    const frequencyText = getFrequencyText(frequency);
    if (frequency === "monthly") {
      return `${t("recurring.dayOf")} ${recurring.dayOfMonth} ${t("recurring.every")} ${frequencyText.toLowerCase()}`;
    }
  ...
  return (
    <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateNew("dayOfMonth", e.target.value)}
                      className={`w-full px-4 py-3 rounded-xl border ${inputClass} focus:ring-2 focus:border-transparent`}
                      required
                    />
                  </div>
                      ...
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            >
                              {recurring.active ? t("recurring.active") : t("recurring.paused")}
                            </span>
                          </div>
                          <div
      ...
      <EditRecurringDialog
        ...
        onSubmitEditRecurring={onSubmitEditRecurring} // (payload)
